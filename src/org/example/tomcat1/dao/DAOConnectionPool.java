...
public final class DAOConnectionPool {
	private static final DAOConnectionPool INSTANCE =
			new DAOConnectionPool();
	...
	private DAOConnectionPool() {
		DAOResourceManager daoResourseManager = 
				DAOResourceManager.getInstance();	
		
		this.driver = daoResourseManager.getValue(DB_DRIVER);
		this.host = daoResourseManager.getValue(CONNECTION_HOST);
		this.login = daoResourseManager.getValue(CONNECTION_LOGIN);		
		this.password = daoResourseManager.getValue(CONNECTION_PASSWORD);

		try {
			this.poolSize = Integer.parseInt(daoResourseManager
					.getValue(CONNECTION_POOL_SIZE));
	...
	public void closeConnection(Connection con,
			Statement st) throws DAOException {
		try {
			con.close();
		} catch (SQLException e) {
			throw new DAOException(e);
